...
const (
	BroadcastRanksUpdated = "BroadcastRanksUpdated"
)

func BroadcastRanksUpdatedStart() {
	...
	go func() {
		for range ticker.C {
			if err = pingProducer.BroadcastRanksUpdated(context.Background(), BroadcastRanksUpdated, ping.RanksUpdatedMsg{
	...
	go func() {
		ch, errBroadcastPing := pingConsumer.BroadcastPing(context.Background(), BroadcastRanksUpdated)
